...
bool processSample(const int16_t *sample, const uint64_t sampleSize, uint64_t lowFreq, uint64_t highFreq, double threshold) {
	...
	try {
		for (int i = 0; i < nSamples; i += sampleSize) {
			if (i + sampleSize >= nSamples)
				break;
			if (processSample(&samples[i], sampleSize, lowFreqS, highFreqS, threshold)) {
